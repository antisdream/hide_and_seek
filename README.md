...
- 4~10명 공개 빠른 매칭
- 빠른 매칭과 분리된 하나의 `친구·AI 방 만들기`: 쉬움·보통·어려움을 고르면 AI가 빈자리를 채우고 친구가 들어올 때 한 명씩 교대
- 초대 링크 또는 초대 코드를 직접 입력해 참여하는 친구 기능
...
- 틈새정령 6.5·관찰자 9.5의 빠른 이동, 약 46% 추격 우위와 프레임 연속 로컬 이동·30Hz 서버 좌표 시간 보간
- 직전 라운드 관찰자를 연속 배정하지 않는 역할 순환과 20점 단위 생존 점수
...
- 사람과 AI를 같은 역할 이력으로 관리해 직전 라운드 관찰자는 다음 라운드에 연속 배정하지 않고, 세 라운드에서 역할이 순환하게 했습니다.
...
### 0.012 · 2026-08-22 기준 · 사람·AI 혼합 밸런스 검증 예정
...
- 직접 만든 방에서 사람 1명, 사람 2명+AI 2명, 사람 3명+AI 1명 조합으로 세 난이도를 각각 플레이합니다.
...
- AI 관찰자의 발견률과 AI 틈새정령의 생존 점수가 난이도 순서에 맞게 변하는지 기록합니다.
...
3. 혼자 또는 친구와 하려면 쉬움·보통·어려움 중 하나를 고르고 `친구·AI 방 만들기`를 선택합니다.
4. `준비 완료`를 누르면 부족한 자리를 AI가 채워 경기가 시작됩니다. 표시되는 초대 링크로 친구가 들어오면 AI가 한 명씩 자동으로 빠집니다.
5. 링크를 열기 어렵다면 받은 초대 코드나 링크를 입장 화면의 `초대 코드로 참가`에 붙여 넣습니다.
6. 공개 이용자와 플레이하려면 `빠른 매칭`을 선택합니다.
...
| 역할 | 목표 | 주요 행동 |
|---|---|---|
| 틈새정령 | 수색 종료까지 한 명 이상 생존 | 이동, 위치 고정, 맵 전체 무작위 자리바꿈 1회, 진열 미션, 팀 핑 |
...
틈새정령은 초당 6.5칸, 관찰자는 초당 9.5칸을 이동합니다. 방향이 바뀌면 입력을 즉시 전송하고 0.1초마다 유지 상태를 확인합니다. 서버는 초당 30회 판정하며, 내 캐릭터는 서버와 같은 충돌 규칙으로 화면 프레임마다 움직인 뒤 정지할 때 작은 좌표 차이를 정리하고 다른 참가자는 서버 시간 기준으로 프레임 사이를 연결합니다. 확인 스티커는 정확한 확인 뒤 1.2초, 잘못된 확인 뒤 3초, 집중력을 모두 쓴 뒤 6.5초 동안 다시 사용할 수 없습니다. 게임 수치의 기준은 [`shared/game-rules.ts`](shared/game-rules.ts)입니다.
...
AI는 별도 머신러닝 모델을 훈련하거나 접속자의 데이터를 학습하지 않습니다. 서버에서 현재 라운드의 시야 안 단서와 짧은 기억만 사용하는 행동 AI이므로, 화면 밖이나 선반 너머에 숨은 정답 위치를 미리 알 수 없습니다. 세부 프로필은 [`shared/ai-rules.ts`](shared/ai-rules.ts)에 있습니다.
