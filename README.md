...
- 쉬움·보통·어려움 중 난이도를 고르고, 사람 수에 맞춰 AI가 네 자리를 채우는 AI 방
...
- 틈새정령 6.5·관찰자 9.5의 빠른 이동, 약 46% 추격 우위와 로컬 예측·30Hz 서버 좌표 시간 보간
...
### 0.011 · 2026-08-21 기준 · 사람·AI 혼합 밸런스 검증 예정
...
- AI 방에서 사람 1명, 사람 2명+AI 2명, 사람 3명+AI 1명 조합으로 세 난이도를 각각 플레이합니다.
...
- 로컬 입력 예측과 50ms 원격 보간 지연이 실제 무선 네트워크에서도 순간이동이나 벽 끼임 없이 보이는지 확인합니다.
...
3. 혼자 또는 AI와 함께 확인하려면 쉬움·보통·어려움 중 하나를 고르고 `AI 방 만들기`를 선택합니다.
4. `준비 완료`를 누르면 부족한 자리를 AI가 채워 경기가 시작됩니다. 이 방의 초대 링크로 친구가 들어오면 AI가 한 명씩 자동으로 빠집니다.
...
틈새정령은 초당 6.5칸, 관찰자는 초당 9.5칸을 이동합니다. 방향이 바뀌면 입력을 즉시 전송하고 0.1초마다 유지 상태를 확인합니다. 서버는 초당 30회 판정하며, 내 캐릭터는 로컬 예측 후 서버 좌표로 보정하고 다른 참가자는 서버 시간 기준으로 프레임 사이를 연결합니다. 확인 스티커는 정확한 확인 뒤 1.2초, 잘못된 확인 뒤 3초, 집중력을 모두 쓴 뒤 6.5초 동안 다시 사용할 수 없습니다. 게임 수치의 기준은 [`shared/game-rules.ts`](shared/game-rules.ts)입니다.
